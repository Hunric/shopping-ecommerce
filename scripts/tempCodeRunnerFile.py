...
def get_dummy_json_data():
    """从DummyJSON API获取商品数据"""
    try:
        response = requests.get(DUMMY_JSON_API_URL)
        if response.status_code == 200:
            data = response.json()
            # DummyJSON返回的数据结构是 {"products": [...], "total": 100, "skip": 0, "limit": 30}
...
def process_api_data(api_data):
    ...
    if api_data:
        for item in api_data:
            # DummyJSON有更多分类，我们根据category映射到手机或电脑
            category = item.get('category', '').lower()
            
            # 映射分类
            if any(keyword in category for keyword in ['smartphone', 'mobile', 'phone']):
                category_id = 2  # 手机
                unit = "台"
            elif any(keyword in category for keyword in ['laptop', 'computer', 'tablet']):
                category_id = 3  # 电脑
                unit = "台"
            else:
                # 其他电子产品随机分配
